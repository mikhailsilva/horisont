...
Ревизия: 2026-09-25. Карта из приложенного скилла (2026-09-22); наличие серверов и число инструментов перепроверены через `*_list_tools` 2026-09-25. Каталоги меняются — перед вызовом искать инструмент в текущем сеансе.
...
`search_cloudflare_documentation`. Удаления — только по явной команде.

## Server 2 — Vercel (245 инструментов на 2026-09-25)
...
## Server 3 — Origene научный шлюз (3574 инструмента на 2026-09-25)

Агрегатор саб-серверов, имена `server_N#tool`. Проверить наличие неймспейса в текущем сеансе; в приложенной карте были 2–21, 23–45, 47–49.
...
| 32 | SCP-Workflow | `generate_presigned_url` (файловое хранилище), `query_and_filter_tools(prompt)` — семантический поиск по всем инструментам |
...
Данные приложенной карты об отсутствии отдельных функций в server 3 — не гарантия на будущее: искать по актуальному каталогу, не делать выводов об отсутствии по одному запросу.
...
с большим limit (помнить про output budget). Вызов возможен только если сервер
предоставлен инструментами текущего сеанса.
